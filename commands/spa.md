---
description: Read text aloud via Bolo with American voice (am_michael). Local-only wrapper, identical HUD subtitle behavior as /speak. `/spa <text>` reads now. `/spa last` reads your last response. `/spa now` forces read of last. `/spa stop` kills audio. `/spa skip` skips next auto-read. `/spa auto on|off` toggles persistent auto-read pinned to the American voice.
---

## Your Task

Parse the user's `/spa` arguments and dispatch through Bolo with the American voice override (`am_michael`). All audio runs through the same Bolo binary as `/speak`, so the live HUD subtitle works here too.

`SPA_VOICE` defaults to `am_michael`. To change permanently, edit this file or use `/voice <name>` after a /spa invocation.
...
```bash
( echo "<your last response text>" | "${BOLO_HOME:-$HOME/.local/share/bolo}/bin/bolo" --voice am_michael >/dev/null 2>/dev/$(ps -o tty= -p $PPID | tr -d ' ') ) &
```
...
```bash
( "${BOLO_HOME:-$HOME/.local/share/bolo}/bin/bolo" --voice am_michael "<text from user>" >/dev/null 2>/dev/$(ps -o tty= -p $PPID | tr -d ' ') ) &
```
...
echo "  /spa <text>      — speak text now (am_michael)"
...
- Voice override is per-invocation via `--voice am_michael`. `auto on` also pins it in `config.json` so the stop-hook auto-read uses it.
